...
void app_main()
{
    ...
    esp_rmaker_node_t *node = esp_rmaker_node_init(&rainmaker_cfg, "ESP RainMaker Multi Device", "Maia RainMaker template");
    if (!node) {
    ...
    temp_sensor_device = esp_rmaker_temp_sensor_device_create("Temperature Sensor", NULL, getTemperature());
    esp_rmaker_node_add_device(node, temp_sensor_device);


    /* Create a device and add the relevant parameters to it */
    esp_rmaker_device_t *gpio_device = esp_rmaker_device_create("Maia RainMaker template", NULL, NULL);
